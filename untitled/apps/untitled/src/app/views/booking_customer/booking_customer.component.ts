...
export class Booking_customerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.homeService
      .create(vehicleItems)
      .subscribe((response) => (this.vehicleItems = response.data.list));
    // console.log(this.encodedUrl);
    // console.log(this.currentUrl);
  }
  private getTypeList() {
    this.vehicleService
      .getTypeVehicleList(this)
      .subscribe((response) => (this.vehicleTypes = response.data.list));
  }
  private getSeat() {
    this.vehicleService
      .getSeatVehicleList(this)
      .subscribe((response) => (this.seats = response.data.list));
  }
  private getCompany() {
    this.vehicleService
      .getCompanyVehicleList(this)
      .subscribe((response) => (this.companies = response.data.list));
  }
  private getFuel() {
    this.vehicleService
      .getFuelVehicleList(this)
      .subscribe((response) => (this.fuels = response.data.list));
  }
  private getShop() {
    this.vehicleService
      .getShopVehicleList(this)
      .subscribe((response) => (this.shops = response.data.list));
  }
  // private getItem(){
